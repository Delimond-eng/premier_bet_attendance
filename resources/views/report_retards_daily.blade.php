    ...
    <div class="content" id="App" v-cloak>
        <div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
            <div class="my-auto mb-2">
                <h2 class="mb-1">Rapport des retards</h2>
                <nav>
        ...
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                <h5>Liste des retards (@{{ range.from }} -> @{{ range.to }})</h5>
                ...
                <div class="table-responsive">
                    <table class="table" ref="table">
                        ...
                        <tr v-for="r in rows" :key="r.key">
                            ...
                            <td><span class="badge badge-soft-warning">@{{ r.expected_time ?? '--:--' }}</span></td>
                            <td><span class="badge badge-soft-info">@{{ r.arrival_time ?? '--:--' }}</span></td>
                            <td><span class="badge badge-soft-danger">@{{ r.late_minutes ?? 0 }}</span></td>
                            <td>
                                <span class="badge" :class="(r.justificatif || '').startsWith('justifie') ? 'badge-info' : 'badge-danger'">
                                    @{{ r.justificatif ?? 'non justifie' }}
                                </span>
                            </td>
                        </tr>
                        </tbody>
...
@push("scripts")
    <script type="module" src="{{ asset("assets/js/scripts/report-retards-daily.js") }}"></script>
@endpush
